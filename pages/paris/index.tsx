...
//import NFT from "../../artifacts/contracts/knft.sol/knft.json";


const nftContractAddress = "0xEf3e790e3cECf0142CD85e1926Af89A6c75a8B63";
...
export default function Paris() {
	...
	const checkIfWalletIsConnected = async () => {
		...
		if (accounts?.length !== 0) {
			...
			setCurrentAccount(String(accounts?.[0]));
	...
	const connectWallet = async () => {
		...
		try {
			...
			*/
			...
			const mumbaiChainId = "0x3e9"; // baobab
			
			if (chainId !== mumbaiChainId) {
				//alert('You are not connected to the Mumbai!');
				alert('You are not connected to the Baobab!');
				return
			}
		...
		try {
			...
			if (ethereum) {
				...
				*/
				...
				const nftTx = await nftContract.stake([366]);
	...
	return (
		<div className='flex flex-col items-center pt-32 bg-[#f3f6f4] text-[#6a50aa] min-h-screen'>
			...
			{currentAccount === '' ? (
				...
				<div>----------------------------------------</div>
				<div>Please connect to the Baobab Testnet</div>
